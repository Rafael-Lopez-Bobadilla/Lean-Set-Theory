...
theorem quotient_is_partition
  ...
  (A/R) is a partition of A := by
  have h1: ∀d: Set, d∈A → ∃S: Set, S∈(A/R) ∧ d∈S := by
    intro d h2
    have h3: (d,d)∈R := (h0.right.left.right) d h2
    have h4 := (equivalence_class R A d h0 h2 d).mpr h3
    have h5 := (quotient_set R A h0 ([R,A]class(d))).mpr
      ⟨d, h2, rfl⟩
    exact ⟨[R,A]class(d), h5, h4⟩
  have h2: ∀S T: Set, S∈(A/R) ∧ T∈(A/R) ∧ S≠T → S∩T=∅ := by
    intro S T h3
